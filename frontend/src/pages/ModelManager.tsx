...
import { Download, Trash2, RefreshCw, MessageSquare, Check, Power, AlertCircle, HardDrive } from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import { useAppStore } from '../store/useAppStore'
import { getModels, deleteModel, getCheckpoints, getLoras } from '../lib/api'
import type { Server, CheckpointList, ModelFileList } from '../lib/api'
...
export function ModelManager() {
  ...
  const loadGpuModels = async () => {
    ...
    setGpuError(null)
    try {
      const [cp, lo] = await Promise.all([getCheckpoints(), getLoras()])
      setCheckpoints(cp)
      setLoras(lo)
    } catch (e: unknown) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ flex: 1, overflow: 'auto', padding: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          <button style={tabBtn('gpu')} onClick={() => setTab('gpu')}>
            GPU Models
          </button>
        ...
        {tab === 'gpu' && (
          <>
            {gpuLoading && <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>Loading…</div>}

            {gpuError && (
              <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 12, color: 'var(--danger)' }}>
                Couldn't load GPU model files: {gpuError}
              </div>
            )}

            {!gpuLoading && !gpuError && checkpoints && loras && (
              <>
                <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>
                  Read-only for now — reflects whatever's actually on disk for Stable Diffusion.
                  Delete/install aren't wired up here yet.
                </div>

                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <HardDrive size={14} color="var(--text-muted)" />
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)' }}>
                    Checkpoints ({checkpoints.files.length}, {formatSize(checkpoints.total_bytes)})
                  </span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
                  {checkpoints.files.map(f => (
                    <div key={f.filename} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 600, color: 'var(--text)', fontSize: 13 }}>{f.filename}</div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{formatSize(f.size_bytes)}</div>
                      </div>
                      {f.style ? (
                        <span style={{ fontSize: 10, background: 'rgba(34,197,94,0.15)', color: 'var(--success)', borderRadius: 4, padding: '2px 8px', fontWeight: 600 }}>
                          style: {f.style}
                        </span>
                      ) : (
                        <span style={{ fontSize: 10, background: 'var(--surface)', color: 'var(--text-muted)', borderRadius: 4, padding: '2px 8px', border: '1px solid var(--border)' }}>
                          not wired to a style
                        </span>
                      )}
                    </div>
                  ))}
                  {checkpoints.files.length === 0 && (
                    <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>No checkpoint files found.</div>
                  )}
                </div>

                {checkpoints.missing_from_styles.length > 0 && (
                  <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, padding: '10px 14px', marginBottom: 24, fontSize: 12, color: 'var(--danger)' }}>
                    Styles pointing at a missing file: {checkpoints.missing_from_styles.map(m => `${m.style} → ${m.filename}`).join(', ')}
                  </div>
                )}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  <HardDrive size={14} color="var(--text-muted)" />
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text)' }}>
                    LoRAs ({loras.files.length}, {formatSize(loras.total_bytes)})
                  </span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {loras.files.map(f => (
                    <div key={f.filename} style={{ background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 16px' }}>
                      <div style={{ fontWeight: 600, color: 'var(--text)', fontSize: 13 }}>{f.filename}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{formatSize(f.size_bytes)}</div>
                    </div>
                  ))}
                  {loras.files.length === 0 && (
                    <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>No LoRA files found.</div>
                  )}
                </div>
              </>
            )}
