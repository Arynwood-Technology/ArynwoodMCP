...
def _public_url(target: dict, remote_path: str) -> str:
    """Build the public HTTP URL for a remote file, using public_url base or IPv6 fallback."""
...
async def list_targets(db=Depends(get_db)):
    """GET /targets — list all SFTP deployment targets; passwords are masked."""
...
async def create_target(t: TargetIn, db=Depends(get_db)):
    """POST /targets — create a new SFTP deployment target and return its id."""
    await db.execute(
...
async def update_target(tid: int, t: TargetIn, db=Depends(get_db)):
    """PATCH /targets/{tid} — update a target; skips password if the masked placeholder is sent."""
...
async def delete_target(tid: int, db=Depends(get_db)):
    """DELETE /targets/{tid} — remove a deployment target from the database."""
...
async def test_connection(tid: int, db=Depends(get_db)):
    """POST /targets/{tid}/test — verify SSH/SFTP connectivity and list file count in web_root."""
    ...
    def _test():
        """Connect over SFTP and verify that web_root is listable."""
...
async def browse(tid: int, path: str = "", db=Depends(get_db)):
    """GET /targets/{tid}/browse — list directory contents at the given remote path."""
    async with db.execute("SELECT * FROM deploy_targets WHERE id=?", (tid,)) as cur:
        row = await cur.fetchone()
    if not row:
    ...
    def _browse():
        """SFTP-list a directory and return sorted entry metadata."""
        ssh, sftp = _open_sftp(target)
        try:
            entries = []
...
async def upload_file(
    ...
    """POST /targets/{tid}/upload — upload a file to the remote server via SFTP."""
    async with db.execute("SELECT * FROM deploy_targets WHERE id=?", (tid,)) as cur:
        row = await cur.fetchone()
    if not row:
        raise HTTPException(404)
    ...
    def _upload():
        """Write file bytes to the remote path, creating parent directories as needed."""
        ssh, sftp = _open_sftp(target)
        try:
...
def _sftp_makedirs(sftp: paramiko.SFTPClient, path: str):
    """Recursively create directories on the remote via SFTP, skipping existing ones."""
    parts = path.split("/")
    current = ""
    for part in parts:
        if not part:
...
async def delete_file(tid: int, remote_path: str, db=Depends(get_db)):
    """DELETE /targets/{tid}/file — remove a single file from the remote server."""
    async with db.execute("SELECT * FROM deploy_targets WHERE id=?", (tid,)) as cur:
        row = await cur.fetchone()
    if not row:
        raise HTTPException(404)
...
async def make_dir(tid: int, path: str = Form(...), db=Depends(get_db)):
    """POST /targets/{tid}/mkdir — create a new directory on the remote server."""
    async with db.execute("SELECT * FROM deploy_targets WHERE id=?", (tid,)) as cur:
        row = await cur.fetchone()
    if not row:
        raise HTTPException(404)
    ...
    def _mkdir():
        """Create the directory at the safe remote path via SFTP."""
        ssh, sftp = _open_sftp(target)
        try:
            sftp.mkdir(safe)
        finally:
...
async def publish_page(
    ...
    """POST /targets/{tid}/publish-page — render a styled HTML page and upload it via SFTP."""
    async with db.execute("SELECT * FROM deploy_targets WHERE id=?", (tid,)) as cur:
        row = await cur.fetchone()
    if not row:
        raise HTTPException(404)
...
<body>
  ...
  <footer>Published with <a href="https://github.com/aryn" target="_blank">Arynwood MCP</a></footer>
</body>
</html>"""
    ...
    def _publish():
        """Write the generated HTML bytes to the remote path via SFTP."""
        ssh, sftp = _open_sftp(target)
        try:
