# Arynwood implementation tracker

Requested filename: `inprovements.md`. Updated 2026-09-23.

This is the complete improvement backlog and execution record. The full audit follows the checklist. Checked items require implementation and relevant validation; partial items remain unchecked with their actual scope recorded. Live-service and packaged-build checks are tracked separately from unit tests.

## Execution checklist

- [x] 1. Preserve structured model events
- [x] 2. Budget the complete request at every model call
- [x] 3. Summarize what actually leaves context
- [x] 4. Preserve confirmed memory when proposing changes
- [x] 5. Apply memory trust and expiry consistently
- [x] 6. Make knowledge replacement and deletion reliable
- [x] 7. Make failures distinguishable from empty evidence
- [x] 8. Introduce provider adapters and model capability profiles
- [x] 9. Carry task state into tool execution
- [x] 10. Add durable task execution state
- [x] 11. Correct tool-result handling and verification
- [ ] 12. Improve tool discovery, validation, and policy
- [x] 13. Unify the HTTP and WebSocket runtimes
- [ ] 14. Replace prompt contradictions with live capabilities
- [x] 15. Make projects an actual context boundary
- [ ] 16. Separate kinds of memory
- [x] 17. Upgrade retrieval candidate generation
- [ ] 18. Preserve document structure and embedding identity
- [x] 19. Make web research citable
- [ ] 20. Connect the assistant to actual studio work
- [x] 21. Persist evidence and completion events
- [x] 22. Stop presenting similarity as confidence
- [x] 23. Add cancellation and structured message dispatch
- [ ] 24. Coordinate all local inference with GPU work
- [ ] 25. Evaluate behavior across models, including failures

## Execution log

- Baseline: 62 focused AI-runtime tests passed during audit. Existing frontend demo/studio edits were preserved.
- Implemented: normalized structured streaming events, provider adapter support, whole-request budgeting, token-eviction summaries, revisioned memory proposals, scoped retrieval, durable index retries, staged knowledge activation, typed MCP results, run/evidence persistence, project selection, cancellation, and citable web evidence.
- Validation: 307 backend tests passed, 20 live-model tests were deselected by the repository default, and the production frontend build passed. One existing Starlette/httpx deprecation warning remains.
- Still open: full MCP protocol negotiation/pagination, GPU-wide inference scheduling, studio workflow tools, structured memory categories, prompt capability cleanup, and live Ollama/Qdrant/package verification.

### Live verification — 2026-09-23 (for 0.4.4)

Ran a source backend on a copy of the DB against real Ollama 0.11.4 (RTX 3060 12GB) and a throwaway Qdrant container, talked to all five personas over the real WebSocket, and drove the Chat/Dashboard UI in Chrome. Verified working: project-scoped memory and knowledge (no cross-project leakage), confirmed memory preserved when a change is proposed (conflict/revision, accept/reject endpoints), knowledge versioned replace + durable vector cleanup + delete without resurrecting the old version, HTTP `/complete` parity, the persisted run/evidence trail, cancellation. All 13 routes load with no console errors or failing API calls.

Found and fixed (regression tests in `tests/test_live_verification_fixes.py`, each confirmed to fail with its fix reverted):

- **Arynwood never saw its memories.** Estimated tokens overcounted real ones by 48% and the system prompt had a fixed 50% share, so relevant memories were always trimmed; the model then invented "decisions" and saved them. Token estimates now calibrate from Ollama's real counts (persisted), the system prompt is sized from what history/evidence leave, and an explicit "no saved memories matched — don't invent one" line is always present. Result: 3/3 honest "no record" answers in an empty project, correct recall in the other.
- **Model reloads every turn.** GPU embeddings evicted the 14B chat model and utility calls without `num_ctx` forced reloads. Embeddings now run on CPU; unspecified `num_ctx` reuses the resident value. Warm time-to-first-token ~21s → ~9s.
- **Web search outages looked like "no results"** (ddgs raises intermittently on news; the fallback was skipped). Now typed: `WebSearchUnavailable` vs. empty.
- **Codebase gate misrouted every Kdenlive inspection question** (4/4). Multiple enabled servers now route with one side-by-side call (12/12 + follow-ups; `ROUTE_CASES` evals); the codebase server is only offered when `ARYNWOOD_ENABLE_CODEBASE_TOOLS=1`.
- **Stale facts re-fed from other chats**: recent-conversation context replayed Arynwood's own past answers (one quoting a deleted knowledge source), self-reinforcing. It now carries only the user's side.
- Stop discarded streamed text (now saved as `*(stopped)*`, once); UI appended the next reply to the stopped text; Stop button scrolled out of view (moved into the composer).
- Capability overclaiming (item 14, partial): "Available capabilities" is generated from the persona's real tools; Glyph/Doc/Kona/Estra no longer say "You have no real tools" while having some; central's prompt no longer implies it can run GPU/studio features itself.
- Smaller: spurious "Preserving earlier conversation details" + false `summary: unavailable` on first turns; empty knowledge base reported as "semantic search offline"; Knowledge page search couldn't find project-scoped sources (search is cross-project unless `project_id` is passed); no-op memory revisions from restated memories; evidence panel was raw JSON and lost on reload (now a readable "What informed this reply", loaded from `/runs`); empty reply bubble when a reply was only a memory save; memory panel rows squashed.

Still not verified live: the destructive-tool approval round-trip against a running Kdenlive (needs a scratch project so an approved call can't touch real work), the packaged AppImage build, and non-Ollama providers.

---

# Arynwood AI runtime improvement audit

Reviewed 2026-09-23. Scope: source-level review of chat, providers, memory, knowledge retrieval, MCP execution, relevant UI flows, and tests. This is not a live quality benchmark or an exhaustive review of every studio integration. Application code was changed only in the working tree; no live user data was changed.
